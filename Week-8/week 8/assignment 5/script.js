...
for (let i = 0; i < trust.length; i++) node.push(...trust[i]);
node = new Set(node);
node = [...node];

for (let i = 0; i < trust.length; i++) addEdge(adj, trust[i]);
