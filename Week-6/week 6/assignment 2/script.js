function matrix2(arr) {
  ...
  while (startColumn <= endColumn && startRow <= endRow) {
    ...
    for (let i = startRow; i <= endRow; i++) result.push(arr[i][endColumn]);

    endColumn--;

    // Bottom row
    //Push all the element in end row and then decrement endRow pointer to previous row
    for (let i = endColumn; i >= startColumn; i--) result.push(arr[endRow][i]);

    endRow--;

    // left column
    //Push all the element in left column and then increment startColumn pointer to next column
    for (let i = endRow; i >= startRow; i--) result.push(arr[i][startColumn]);
